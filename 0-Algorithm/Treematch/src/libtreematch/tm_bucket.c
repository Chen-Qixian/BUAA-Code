...
{
  tm_verbose_fprintf(DEBUG, verbose_stdout, "\tb.bucket=%p\n",(void *)b->bucket);
  tm_verbose_fprintf(DEBUG, verbose_stdout, "\tb.bucket_len=%d\n",(int)b->bucket_len);
  tm_verbose_fprintf(DEBUG, verbose_stdout, "\tb.nb_elem=%d\n",(int)b->nb_elem);
}
...
void display_bucket_list(bucket_list_t bucket_list)
{
  ...
  for(i = 0 ; i < bucket_list->nb_buckets ; i++){
    ...
    if( i == bucket_list->nb_buckets - 1 )
      inf = 0;
    if(verbose_level >= DEBUG){
      tm_verbose_fprintf(DEBUG,verbose_stdout,"Bucket %d:\n",i);
...
void add_to_bucket(int id,int i,int j,bucket_list_t bucket_list)
{
  ...
  if( bucket->bucket_len == bucket->nb_elem ){
    ...
    size = N*N/n;
    /* display_bucket(bucket);*/
    tm_verbose_fprintf(DEBUG,verbose_stdout,"Extending bucket %d (%p) from size %lu to size %lu!\n",id,(void*)bucket->bucket, (unsigned long) bucket->nb_elem, (unsigned long)bucket->nb_elem+size);
...
void partial_sort(bucket_list_t *bl,double **tab,int N)
{
  ...
  n = pow(nb_buckets,2);
  tm_verbose_fprintf(INFO,verbose_stdout,"N=%d, n=%d\n",N,n);
