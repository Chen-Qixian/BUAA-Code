/* /home/chun/mpiP-3.4.1/wrappers.c */
/* DO NOT EDIT -- AUTOMATICALLY GENERATED! */
/* Timestamp: 16 January 2018 16:07  */
/* Location: ubuntu posix */
/* Creator: chun  */


#include "mpiPi.h"
#include "symbols.h"
#include "weak-symbols.h"
#include "mpiPi_def.h"
#include <stdio.h>
#include <stdlib.h>
#include <strings.h>
//extern int **Comm_Times;
int MPI_TYPE[55] = {1275070475,1275069445,1275068673,1275068931,1275070471,1275069450,
1275068685,1275070473,1275068696,1275068674,1275068932,1275069446,
1275070472,1275070489,1275072524,1275069454,1275068735,1275068727,
1275068984,1275069497,1275070522,1275068731,1275068988,1275069501,
1275070526,1275070528,1275072577,1275076674,1275070531,1275070532,
-1946157056,-1946157054,-1946157055,-1946157053,1275070486,1275070497,
1275072547,-100,1275069468,1275069479,1275070505,1275072555,1275070504,
1275072554,1275076652,1275068717,1275068975,1275069488,1275070513,201326592,
-1946157052,1275068433,1275068432,1275068687,-100
};

int MPITYPE_Trans(int MPI_TYPE_Code)
{
	int i =0;
	for(i=0;i<55;i++)
    {
        if(MPI_TYPE[i] == MPI_TYPE_Code)
        {
            return i;
        }		
    }
	switch(MPI_TYPE_Code)
	{
		case 201326592:{return -1; break;}
		case 1275070495:{return 0; break;}
		case 1275068698:{return 2; break;}
		case 1275069469:{return 2; break;}
		case 1275070533:{return 2; break;}
		case 1275070473:{return 7; break;}
		case 1275070494:{return 25; break;}
		case 1275070528:{return 25; break;}
		case 1275072546:{return 26; break;}
		case 1275070496:{return 34; break;}
...
static int mpiPif_MPI_Allreduce( jmp_buf * base_jbuf, mpip_const_void_t  *sendbuf, void  *recvbuf, int  * count, MPI_Datatype  * datatype, MPI_Op  * op, MPI_Comm  * comm)
{
 ...
 /****set trace file path and open it****/
 char log_path[50] = "./CommData_";
 char rankString[30];
 int my_rank, mpi_err;
 mpi_err = PMPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
 sprintf(rankString,"%d",my_rank);
 strcat(log_path,rankString);
 trace_file_ptr = fopen(log_path,"a+");
	
 /****printf trace context****/
 char trace_date[100];   
 //long int send_size = 0;
 //send_size = (*count) * sizeof(* datatype); 
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d allreduce %d 0 0 %d\n",my_rank,*count,mpi_datatype_code);
 fprintf(trace_file_ptr,"%s", trace_date);  
 /****close trace file****/
...
static int mpiPif_MPI_Barrier( jmp_buf * base_jbuf, MPI_Comm  * comm)
{
 ...
 trace_file_ptr = fopen(log_path,"a+");
 /****printf trace context****/
 fprintf(trace_file_ptr,"%d barrier\n", my_rank);  
 /****close trace file****/
 fclose(trace_file_ptr);
...
static int mpiPif_MPI_Bcast( jmp_buf * base_jbuf, void  *buffer, int  * count, MPI_Datatype  * datatype, int  * root, MPI_Comm  * comm)
{
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d bcast %d 0 %d\n",my_rank,*count,mpi_datatype_code);
 fprintf(trace_file_ptr,"%s", trace_date);  
 /****close trace file****/
 fclose(trace_file_ptr);	
...
static int mpiPif_MPI_Irecv( jmp_buf * base_jbuf, void  *buf, int  * count, MPI_Datatype  * datatype, int  * source, int  * tag, MPI_Comm  * comm, MPI_Request  *request)
{
 ...
 mpiPi_TIME start, end;
 void *call_stack[MPIP_CALLSITE_STACK_DEPTH_MAX] = { NULL };
 /*char OpType[20] = "irecv:";
 char destNode[5];
 sprintf(destNode,"%d",*source);
 strcat(OpType,destNode);
 //logfile_ptr = fopen(log_path, "w");
 fputs(OpType,logfile_ptr);
 fputc(13,logfile_ptr);*/
 /*int my_rank, mpi_err;
 mpi_err = PMPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
 Comm_Matrix[*source][my_rank] = 1;	
 Comm_Matrix[my_rank][*source] = 1; */
 
 	
   /****set trace file path and open it****/
 char log_path[50] = "./CommData_";
 char ComTimes_path[50]="./ProcessCommTrace_";
 char rankString[30];
 int my_rank, mpi_err;
 mpi_err = PMPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
 sprintf(rankString,"%d",my_rank);
 strcat(log_path,rankString);
 strcat(ComTimes_path,rankString);
 trace_file_ptr = fopen(log_path,"a+");
 CommTimes_ptr = fopen(ComTimes_path,"a+");	
 /****printf trace context****/
 char trace_date[100];   
 char times_date[100];
 long int send_size = 0;
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d Irecv %d %d %d\n",my_rank,*source,*count, mpi_datatype_code);
 sprintf(times_date,"%d %d %ld\n",my_rank,*source,send_size);
 fprintf(trace_file_ptr,"%s", trace_date);  
 fprintf(CommTimes_ptr,"%s",times_date);
 /****close trace file****/
 fclose(trace_file_ptr);		
 fclose(CommTimes_ptr);
 //Comm_Times[*source][my_rank]++;	
 //Comm_Times[my_rank][*source]++;	
...
static int mpiPif_MPI_Isend( jmp_buf * base_jbuf, mpip_const_void_t  *buf, int  * count, MPI_Datatype  * datatype, int  * dest, int  * tag, MPI_Comm  * comm, MPI_Request  *request)
{
 ...
 /*int my_rank, mpi_err;
 mpi_err = PMPI_Comm_rank(MPI_COMM_WORLD, &my_rank);
 Comm_Matrix[my_rank][*dest] = 1;
 Comm_Matrix[*dest][my_rank] = 1;*/
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d Isend %d %d %d\n",my_rank,*dest,*count, mpi_datatype_code);
 sprintf(times_date,"%d %d %ld\n",my_rank,*dest,send_size);
 fprintf(trace_file_ptr,"%s", trace_date);  
 fprintf(trace_file_ptr,"%s", trace_date);
 /****close trace file****/
 fclose(trace_file_ptr);	
 fclose(CommTimes_ptr);
 //Comm_Times[my_rank][*dest]++;	
 //Comm_Times[*dest][my_rank]++;
...
static int mpiPif_MPI_Issend( jmp_buf * base_jbuf, mpip_const_void_t  *buf, int  * count, MPI_Datatype  * datatype, int  * dest, int  * tag, MPI_Comm  * comm, MPI_Request  *request)
{
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d Issend %d %d %d\n",my_rank,*dest,*count, mpi_datatype_code);
 fprintf(trace_file_ptr,"%s", trace_date);  
 /****close trace file****/
 fclose(trace_file_ptr);		
...
static int mpiPif_MPI_Recv( jmp_buf * base_jbuf, void  *buf, int  * count, MPI_Datatype  * datatype, int  * source, int  * tag, MPI_Comm  * comm, MPI_Status  *status)
{
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d recv %d %d %d\n",my_rank,*source,*count, mpi_datatype_code);
 sprintf(times_date,"%d %d %ld\n",my_rank,*source,send_size);
 fprintf(trace_file_ptr,"%s", trace_date);  
 fprintf(trace_file_ptr,"%s", trace_date);
 /****close trace file****/
...
static int mpiPif_MPI_Reduce( jmp_buf * base_jbuf, mpip_const_void_t  *sendbuf, void  *recvbuf, int  * count, MPI_Datatype  * datatype, MPI_Op  * op, int  * root, MPI_Comm  * comm)
{
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d reduce %d 0 0 %d\n",my_rank,*count,mpi_datatype_code);
 fprintf(trace_file_ptr,"%s", trace_date);  
 /****close trace file****/
 fclose(trace_file_ptr);	
...
static int mpiPif_MPI_Send( jmp_buf * base_jbuf, mpip_const_void_t  *buf, int  * count, MPI_Datatype  * datatype, int  * dest, int  * tag, MPI_Comm  * comm)
{
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d send %d %d %d\n",my_rank,*dest,*count, mpi_datatype_code);
 sprintf(times_date,"%d %d %ld\n",my_rank,*dest,send_size);
 fprintf(trace_file_ptr,"%s", trace_date);
 fprintf(CommTimes_ptr,"%s",times_date);
 /****close trace file****/
 ...
 //printf("this is function test : %d\n",t);
 //printf("test: %d\n",data_type);
 //sprintf(trace_date,"%d send %d %d ",my_rank,*dest,*count, );
...
static int mpiPif_MPI_Ssend( jmp_buf * base_jbuf, mpip_const_void_t  *buf, int  * count, MPI_Datatype  * datatype, int  * dest, int  * tag, MPI_Comm  * comm)
{
 ...
 int data_type = (int)(*datatype);
 int mpi_datatype_code = MPITYPE_Trans(data_type);
 sprintf(trace_date,"%d Ssend %d %d %d\n",my_rank,*dest,*count, mpi_datatype_code);
 fprintf(trace_file_ptr,"%s", trace_date);  
 /****close trace file****/
 fclose(trace_file_ptr);	
if (mpiPi.enabled) {
