...
export default {
  ...
  data () {
    ...
    return {
      ...
      introduction: '利用拓扑映射算法进行通信性能优化的主要原理是将存在大量数据通信的进程划分至相同进程组，进而将其布置在数据传输能力强的计算节点中。因此，并行应用的通信局部性特征成为了拓扑映射优化的重要考虑因素。本系统实现了LPMS、QTLS、TopoMapping、APHiD、MPIPP以及TreeMatch六种算法。为了让并行应用的普通使用者能够快速、直观地感知针对给定场景的最佳拓扑映射算法，本系统将这六种算法进行集成，供用户选择使用，结合相应的算法推荐策略，极大提高了并行应用通信性能优化的效率。',
      dropzoneOptions: {
        url: '/api/file',
        thumbnailWidth: 50,
